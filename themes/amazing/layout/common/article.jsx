                ...
                <article class={`card-content article${'direction' in page ? ' ' + page.direction : ''}`} role="article">
                    {page.layout !== 'page' ? <div class="article-meta size-small is-uppercase level is-mobile">
                        <div class="level-left">
                            {/*置顶图标*/}
                            {page.top > 0 ?
                            <div class="level-item tag is-danger" style="background-color: #3273dc;">Pinned</div>:null}
                            {/* Date */}
                        ...
                        {page.updated && page.updated > page.date ?
                            <p class="text-right font1_1">
                                <time datetime={date_xml(page.updated)}>
                                    <strong><em>&nbsp;Last modified:&nbsp;{date(page.updated)}.</em></strong>
                                </time>
                            </p> : null
                        }
                    </div> : null}
                                ...
                                <div class="level-start">
                                    <div class="level-item has-text-grey is-size-7 is-hidden-mobile">
                                        <time datetime={date_xml(page.updated)}><i
                                            class="far fa-calendar-check">&nbsp;Last modified:&nbsp;</i>{date(page.updated)}
                                        </time>
                                    </div>
                                </div> : null
                            }
                    ...
                    <ul class="post-copyright">
                        <li><strong>Title：</strong><a href={url_for(page.permalink)}>{page.title}</a></li>
                        <li><strong>Author：</strong><a href={url_for(config.url)}>{config.author}</a></li>
                        <li><strong>Link：</strong><a href={url_for(page.permalink)}>{config.url+'/'+page.path}</a></li>
                        <li><strong>Copyright Clarify：</strong>Copyright ownership belongs to Shu-Hsien Cho. For reproduced, please specify from this website.
                        </li>
                    </ul>:null}

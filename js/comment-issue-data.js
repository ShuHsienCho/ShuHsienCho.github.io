...
var repoIssuesUrl = "https://api.github.com/repos/ShuHsienCho/blog_comment/issues";
// 对应仓库 clientId、clientSecret 关于这两个参数的安全问题，查看 https://removeif.github.io/2019/09/19/博客源码分享.html#1-热门推荐，最新评论：
var clientId = "2aced629bd05b2f72525";
var clientSecret = "35bf54171534576960bb92b7714ef4993fd0bc85";
    ...
    if (COMMENT_ARR.length > 0) {
        ...
        for (var i = 0; i < COMMENT_ARR.length; i++) {
            ...
            htmlContentWidget +=
                ...
                "<div class='item-header-text'><a href='"+item.userUrl+"' target='_blank'>" + item.userName + "</a>&nbsp;Created at" + getDateDiff(new Date(item.date).getTime()) + "</div>" + "<div class=\"item-text\"><a href =\"" + item.url + '#comment-container' + "\"target=\"_blank\">" + contentStr + "</a></div>" +
                "</div>";
        }
        htmlContentWidget += "</div>"
        $(".body_hot_comment").html(htmlContentWidget);
    } else {
        $(".body_hot_comment").html("No record!");
    }
}
...
$(document).ready(setTimeout(function () { // 延迟1s执行，保证其余的先加载
        ...
        console.log("You could also visit my github page!")
    }
    ,
    500
))
